...
from methods.smart_random_method import SmartRandomMethod
from common.data_structures import ExperimentResult


def run_smart_random_experiment(target_file: str, repetitions: int = 5, max_iter: int = 15):
    print(f"\n{'='*60}")
    print(f"🔬 SmartRandom方法实验")
    ...
    for rep in range(repetitions):
        ...
        method = SmartRandomMethod(target_file)
    ...
    out_file = f"results/smartrandom_{os.path.basename(target_file)}_{exp_result.timestamp}.json"
...
if __name__ == "__main__":
    ...
    run_smart_random_experiment(target, reps, iters)