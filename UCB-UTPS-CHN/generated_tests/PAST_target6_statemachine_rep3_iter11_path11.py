...
@pytest.mark.parametrize("state, counter, action, expected_state, expected_counter, expected_result", [
    ...
    # 测试状态为 RUNNING 且动作是 INCREMENT，counter < 10
    ("RUNNING", 5, "INCREMENT", "RUNNING", 6, "INCREMENTED"),
    # 测试状态为 RUNNING 且动作是 INCREMENT，counter == 9
    ("RUNNING", 9, "INCREMENT", "COMPLETED", 10, "COMPLETED"),
    # 测试状态为 RUNNING 且动作是 INCREMENT，counter == 10（应返回 NO_CHANGE）
    ...
    # 测试 state != 'RUNNING' 的情况（如 IDLE 状态下执行 INCREMENT）
    ("IDLE", 5, "INCREMENT", "IDLE", 5, "NO_CHANGE"),
    # 测试 state != 'RUNNING' 的情况（如 STOPPED 状态下执行 INCREMENT）
    ("STOPPED", 5, "INCREMENT", "STOPPED", 5, "NO_CHANGE"),
    # 测试 state != 'RUNNING' 的情况（如 COMPLETED 状态下执行 INCREMENT）
