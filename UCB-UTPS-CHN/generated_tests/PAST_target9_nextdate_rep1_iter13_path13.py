# 方法: PAST
# 目标文件: target9_nextdate.py
# 条件路径: month == 2
# 重复次数: 1, 迭代: 13
# 生成时间: 2026-04-26 07:02:54

import pytest
from target9_nextdate import next_date

@pytest.mark.parametrize("year, month, day, expected", [
    # 有效日期测试
    (2020, 2, 28, "2020-03-01"),  # 闰年2月最后一天
    (2021, 2, 28, "2021-03-01"),  # 非闰年2月最后一天
    (2020, 2, 1, "2020-02-02"),   # 闰年2月中间一天
    (2021, 2, 1, "2021-02-02"),   # 非闰年2月中间一天
    (2020, 12, 31, "2021-01-01"), # 年末
    (2020, 11, 30, "2020-12-01"), # 月末
    (2020, 1, 1, "2020-01-02"),   # 月初
    # 无效日期测试
    (0, 1, 1, "INVALID_DATE"),   # 年份小于1
    (10000, 1, 1, "INVALID_DATE"),# 年份大于9999
    (2020, 0, 1, "INVALID_DATE"),# 月份小于1
    (2020, 13, 1, "INVALID_DATE"),# 月份大于12
    (2020, 2, 30, "INVALID_DATE"),# 2月超过天数
    (2021, 2, 29, "INVALID_DATE"),# 非闰年2月有29天
    (2020, 4, 31, "INVALID_DATE"),# 4月有31天
    # 边界值测试
    (1, 1, 1, "0001-01-02"),      # 最小年份、月份、日期
    (9999, 12, 31, "0000-01-01"), # 最大年份、月份、日期（应返回下一年，但超出范围）
    # 参数类型错误
    ("2020", 2, 28, "INVALID_DATE"),# 年份不是整数
    (2020, "2", 28, "INVALID_DATE"),# 月份不是整数
    (2020, 2, "28", "INVALID_DATE"),# 日期不是整数
])
def test_next_date(year, month, day, expected):
    result = next_date(year, month, day)
    assert result == expected