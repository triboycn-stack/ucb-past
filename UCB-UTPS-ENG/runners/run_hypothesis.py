...
from methods.hypothesis_method import HypothesisMethod
from common.data_structures import ExperimentResult


def run_hypothesis_experiment(target_file: str, repetitions: int = 5, max_iter: int = 15):
    print(f"\n{'='*60}")
    print(f"🔬 Hypothesis方法实验")
    ...
    for rep in range(repetitions):
        ...
        method = HypothesisMethod(target_file)
    ...
    out_file = f"results/hypothesis_{os.path.basename(target_file)}_{exp_result.timestamp}.json"
...
if __name__ == "__main__":
    ...
    run_hypothesis_experiment(target, reps, iters)