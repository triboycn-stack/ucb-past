...
# 条件路径: not (action == 'RESUME')
# 重复次数: 3, 迭代: 7
# 生成时间: 2026-04-26 06:40:10
...
@pytest.mark.parametrize("state, counter, action, expected_state, expected_counter, expected_result", [
    ...
    ("COMPLETED", 0, "STOP", "COMPLETED", 0, "NO_CHANGE"),
...
def test_transition(state, counter, action, expected_state, expected_counter, expected_result):
    result = transition(state, counter, action)
    assert result == (expected_state, expected_counter, expected_result)